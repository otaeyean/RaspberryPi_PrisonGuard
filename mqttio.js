mqttio.jsvar port = 9001 // mosquitto의 디폴트 웹 포트
...
function onMessageArrived(msg) { // 매개변수 msg는 도착한 MQTT 메시지를 담고 있는 객체
    ...
    if(msg.destinationName == "temperature"){
        ...
        // 도착한 메시지 출력
           var a=  parseInt( msg.payloadString);

        document.getElementById("messages").innerHTML += '<span>현재의 온도는 : '  +parseInt( msg.payloadString) +'ºc'+
                '</span><br/>';
        if(a>20) {
                document.getElementById("messages").innerHTML+='불이다!!!!!!'+'</span><br>';
        }
    }
   ...
   if(msg.destinationName == "humidity"){
            console.log("onMessageArrived: " + msg.payloadString);
        // 도착한 메시지 출력
        document.getElementById("messages").innerHTML += '<span>현재의 습도는 : '  +parseInt( msg.payloadString) +'%'+
                '</span><br/>';
    }
